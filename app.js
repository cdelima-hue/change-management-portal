...
// 1. Renderizar listas nas abas e preencher selects
function renderizarListasBSYProductos() {
  const contBS = document.getElementById('contenedorTabBS');
  const contProd = document.getElementById('contenedorTabProductos');
  const selBS = document.getElementById('selTabBSParaProducto');

  // Recupera ou inicializa os dados
  ...
  if (typeof PRODUCTOS_POR_BS === 'undefined' || typeof PRODUCTOS_POR_BS !== 'object') {
    const prodSaved = localStorage.getItem('nestle_productos_v4');
    window.PRODUCTOS_POR_BS = prodSaved ? JSON.parse(prodSaved) : {
      "Finance": ["Accounts Payable", "General Ledger"],
      "Supply Chain": ["Logistics", "Warehouse"],
      "HR": ["Payroll", "Recruitment"],
      "IT": ["Infrastructure", "Software Support"],
      "Sales & Marketing": ["Campaigns", "CRM"]
    };
  }
  ...
  if (contBS) {
    if (BUSINESS_SERVICES.length === 0) {
      contBS.innerHTML = '<p class="text-xs text-slate-400 col-span-3">Nenhum Business Service cadastrado.</p>';
    } else {
      contBS.innerHTML = BUSINESS_SERVICES.map(bs => `
        <div class="p-3 bg-slate-50 border border-slate-200 rounded-xl flex justify-between items-center shadow-sm">
          <span class="text-xs font-bold text-slate-700">${bs}</span>
          <div class="flex gap-1">
            <button type="button" onclick="editarBS('${bs}')" class="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-lg text-xs transition-all" title="Editar"><i class="fa-solid fa-pen"></i></button>
            <button type="button" onclick="eliminarBS('${bs}')" class="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg text-xs transition-all" title="Excluir"><i class="fa-solid fa-trash"></i></button>
          </div>
        </div>
      `).join('');
    }
  ...
  // Preencher Select de BS na aba de Produtos
  if (selBS) {
    const valorAtual = selBS.value;
    selBS.innerHTML = '<option value="">Selecciona Business Service...</option>' + 
      BUSINESS_SERVICES.map(bs => `<option value="${bs}">${bs}</option>`).join('');
    if (valorAtual && BUSINESS_SERVICES.includes(valorAtual)) {
      selBS.value = valorAtual;
    }
  }
  ...
  if (contProd) {
    let html = '';
    Object.keys(PRODUCTOS_POR_BS).forEach(bs => {
      if (Array.isArray(PRODUCTOS_POR_BS[bs])) {
        PRODUCTOS_POR_BS[bs].forEach(prod => {
          html += `
            <div class="p-3 bg-slate-50 border border-slate-200 rounded-xl flex justify-between items-center shadow-sm">
              <div>
                <span class="block text-xs font-bold text-slate-700">${prod}</span>
                <span class="block text-[10px] text-slate-400 font-medium">BS: ${bs}</span>
              </div>
              <div class="flex gap-1">
                <button type="button" onclick="eliminarProducto('${bs}', '${prod}')" class="p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg text-xs transition-all" title="Excluir"><i class="fa-solid fa-trash"></i></button>
              </div>
            </div>
          `;
        });
    ...
    contProd.innerHTML = html || '<p class="text-xs text-slate-400 col-span-3">Nenhum produto cadastrado.</p>';
  }
}
...
async function agregarBSDesdeTab() {
  const inp = document.getElementById('inpTabNuevoBS');
  const nome = inp ? inp.value.trim() : '';
  if (!nome) return typeof mostrarToast === 'function' ? mostrarToast('Digite o nome do Business Service', 'warning') : alert('Digite o nome do Business Service');
  ...
  if (!BUSINESS_SERVICES.includes(nome)) {
    ...
    renderizarListasBSYProductos();
    if (typeof poblarSelects === 'function') poblarSelects();
    if (typeof mostrarToast === 'function') mostrarToast(`Business Service "${nome}" adicionado com sucesso!`, 'success');
  } else {
...
async function editarBS(nomeAntigo) {
  ...
  if (idx !== -1) {
    ...
    renderizarListasBSYProductos();
    if (typeof poblarSelects === 'function') poblarSelects();
    if (typeof mostrarToast === 'function') mostrarToast('Business Service atualizado com sucesso!', 'success');
  }
}
...
async function eliminarBS(nome) {
  ...
  if (idx !== -1) {
    ...
    renderizarListasBSYProductos();
    if (typeof poblarSelects === 'function') poblarSelects();
    if (typeof mostrarToast === 'function') mostrarToast(`Business Service "${nome}" removido.`, 'warning');
  }
}
...
async function agregarProductoDesdeTab() {
  ...
  if (!bs) return typeof mostrarToast === 'function' ? mostrarToast('Selecione um Business Service', 'warning') : alert('Selecione um Business Service');
  if (!prod) return typeof mostrarToast === 'function' ? mostrarToast('Digite o nome do Produto', 'warning');
  ...
  if (!PRODUCTOS_POR_BS[bs].includes(prod)) {
    ...
    renderizarListasBSYProductos();
    if (typeof poblarSelects === 'function') poblarSelects();
    if (typeof mostrarToast === 'function') mostrarToast(`Produto "${prod}" adicionado com sucesso!`, 'success');
  } else {
    if (typeof mostrarToast === 'function') mostrarToast('Este produto já existe neste Business Service.', 'warning');
  }
}
...
async function eliminarProducto(bs, prod) {
  ...
  if (PRODUCTOS_POR_BS[bs]) {
    ...
    renderizarListasBSYProductos();
    if (typeof poblarSelects === 'function') poblarSelects();
    if (typeof mostrarToast === 'function') mostrarToast(`Produto "${prod}" removido.`, 'warning');
  }
}
