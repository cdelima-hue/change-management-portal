...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  await brandingService.inicializar();
  ...
  // Iniciar sondeo en segundo plano para sincronizar entre múltiples usuarios (cada 15s)
...
function iniciarAutoSincronizacion() {
  ...
  autoSyncTimer = setInterval(async () => {
    if (authService.estaAutenticado() && !dragActive && document.getElementById('modalChange')?.classList.contains('hidden')) {
      try {
...
async function handleCrearRapidoBSSubmit(e) {
  ...
  try {
    const res = await apiClient.createBusinessService({ nombre, pais });
    if (res.success) {
      mostrarToast(`Business Service "${nombre}" guardado en base de datos ✅`, 'success');
      fecharModalCrearRapidoBS();
      await recargarCatalogosCompletos();
      poblarSelectBSFormulario();
      const sel = document.getElementById('inpBusinessService');
...
async function handleEliminarBS(id) {
  ...
  try {
    const res = await apiClient.deleteBusinessService(id);
    if (res.success) {
...
async function handleCrearRapidoProductoSubmit(e) {
  ...
  try {
    const res = await apiClient.createProduct({ nombre, pais });
    if (res.success) {
      mostrarToast(`Producto "${nombre}" guardado en base de datos ✅`, 'success');
...
async function handleEliminarProducto(id) {
  const p = appState.products.find(x => String(x.id) === String(id));
  const nombre = p ? p.nombre : 'este producto';

  if (!confirm(`¿Eliminar el producto "${nombre}" de la base de datos?`)) return;
...
function renderizarTodo() {
  ...
  else if (v==='reportes') reportsManager.renderizar();
...
function renderizarKanban() {
  ...
  filtradas.forEach(ch => {
    ...
    card.innerHTML = `
      ...
      <!-- BADGES TEXTUALES DE SLA Y ETAPA -->
      <div class="flex flex-col gap-1 my-1.5 pt-1 border-t border-slate-100">
        <div class="flex items-center justify-between">
          ${generarBadgeSLAHTML(metricas)}
        </div>
        <div>
          ${generarBadgeEtapaHTML(metricas)}
        </div>
      </div>
...
async function moverChangeDeFase(changeId, nuevaFase) {
  ...
  try {
    await apiClient.updateChange(ch.id, ch);
  ...
  else if (appState.vistaActiva==='reportes') reportsManager.renderizar();
  actualizarIndicadores();
}
...
async function accionCheckpoint(chId, dKey, faseKey) {
  ...
  if (ch[dKey]) {
    ...
    try { await apiClient.updateChange(ch.id, ch); } catch (_) {}
...
function abrirModalEdicion(idOrSpId) {
  ...
  if (badgeSLAEl) {
    badgeSLAEl.innerHTML = generarBadgeSLAHTML(metricas);
  ...
  try {
    if (isUpdate) {
      ...
      if(idx!==-1){
        ...
        data.id=prev.id;
        await apiClient.updateChange(prev.id, data);
...
async function ejecutarImportacionExcel() {
  ...
  for (const ch of excelParsedRows) {
    try { await apiClient.createChange(ch); } catch (_) {}
  }
...
async function eliminarChange(id) {
  ...
  if(idx!==-1){
    ...
    try {
      await apiClient.deleteChange(ch.id);
